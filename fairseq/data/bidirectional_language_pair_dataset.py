...
def collate(
    ...
    if samples[0].get("prev_output_tgt_tokens", None) is not None:
        prev_output_tgt_tokens = merge("prev_output_tgt_tokens", left_pad=left_pad_target)
    elif input_feeding:
        # we create a shifted version of targets for feeding the
        # previous output target token(s) into the next decoder step
        prev_output_tgt_tokens = merge(
            "target",
            left_pad=left_pad_target,
            move_eos_to_beginning=True,
            pad_to_length=pad_to_length["target"]
    ...
    if prev_output_tgt_tokens is not None:
        batch["net_input"]["prev_output_tgt_tokens"] = prev_output_tgt_tokens.index_select(
            0, sort_order
        )
...
class BidirectionalLanguagePairDataset(LanguagePairDataset):
    """
    ...
    def collater(self, samples, pad_to_length=None):
        ...
        Returns:
            dict: a mini-batch with the following keys:
                ...
                - `net_input` (dict): the input to the Model, containing keys:
                  ...
                  - `prev_output_tgt_tokens` (LongTensor): a padded 2D Tensor of
                  ...
                  - `tgt_tokens` (LongTensor): a padded 2D Tensor of tokens in the
                    target sentence of shape `(bsz, tgt_len)`. Padding will appear
                    on the left if *left_pad_target* is ``True``.
                  - `tgt_lengths` (LongTensor): 1D Tensor of the unpadded
                    lengths of each target sentence of shape `(bsz)`
                  - `prev_output_src_tokens` (LongTensor): a padded 2D Tensor of
                    tokens in the source sentence, shifted right by one
                    position for teacher forcing, of shape `(bsz, src_len)`.
                    This key will not be present if *input_feeding* is
                    ``False``.  Padding will appear on the left if
                    *left_pad_target* is ``True``.
        ...
        if self.src_lang_id is not None or self.tgt_lang_id is not None:
            ...
            tgt_tokens = res["net_input"]["tgt_tokens"]
